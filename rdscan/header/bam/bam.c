...
// kmh
int bam_read1_arr( bamFile fp, uint32_t *x, uint8_t *seq, uint8_t *qual, uint32_t *cigar )
{

	int32_t block_len, ret;
//	int32_t i;
	uint32_t  n_cigar, l_qname, l_qseq;
	int32_t data_len, l_aux;
	//int flag=0;
	assert(BAM_CORE_SIZE == 32);
	if ((ret = bam_read(fp, &block_len, 4)) != 4) {
		if (ret == 0) return -1; // normal end-of-file
	...
	if (bam_read(fp, x, BAM_CORE_SIZE) != BAM_CORE_SIZE) return -3;
//	printf( "pos : %d\n", x[1] );
// kmh: defalut bam_is_be = 0 & bam_no_B, so no need???
//	if (bam_is_be) {
//		bam_swap_endian_4p(&block_len);
//		for (i = 0; i < 8; ++i) bam_swap_endian_4p(x + i);
//	}
...
//	if (flag != 0x2)
//	{return -8;}
	data_len = block_len - BAM_CORE_SIZE;
	// l_qname
	l_qname = x[2]&0xff;
	if (bam_read(fp, NULL, l_qname) != l_qname) return -4;

	// n_cigar*4
	n_cigar = x[3]&0xffff;
	if (bam_read(fp, cigar, n_cigar*4) != n_cigar*4) return -5;

	l_qseq = x[4];
	// query seq
	if (bam_read(fp, seq, (l_qseq+1)/2) != (l_qseq+1)/2) return -6;
	// qual seq
	if (bam_read(fp, qual, l_qseq) != l_qseq) return -7;

	// l_aux
	l_aux = data_len - n_cigar * 4 - l_qname - l_qseq - (l_qseq+1)/2;
	if (bam_read(fp, NULL, l_aux) != l_aux) return -8;
...
char *bam_format1_core(const bam_header_t *header, const bam1_t *b, int of)
{
	...
	if (c->l_qseq) {
		for (i = 0; i < c->l_qseq; ++i) kputc(bam_nt16_rev_table[
