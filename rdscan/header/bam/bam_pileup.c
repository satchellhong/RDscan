...
//}
//
//const bam_pileup1_t *bam_plp_auto2(faidx_t *fai, char **target_name, bam_plp_t iter, int *_tid, int *_pos, int *_n_plp)
////const bam_pileup1_t *bam_plp_auto(bam_plp_t iter, int *_tid, int *_pos, int *_n_plp)
//{
//	//const bam_pileup1_t *plp = 0;
//	char cbases[5] = { 'A', 'C', 'G', 'T', 'N' };
//	int cnt = 0;
//	int ref_tid = -1;
//	int ref_len;
//	char *ref=0;
...
//		bam1_core_t *c = &b->core;
//		uint32_t *cigar = bam1_cigar(b);
//		unsigned char *seq = bam1_seq(b);
//		unsigned char *qual =bam1_qual(b);
//		int strand = bam1_strand(b);
//
//		// check if there is nonMatch
...
////				break;
////			}
//			if ( op == BAM_CSOFT_CLIP ) { // BAM_CEQUAL BAM_CDIFF BAM_CDEL BAM_CINS BAM_CREF_SKIP BAM_CSOFT_CLIP BAM_CHARD_CLIP
//				nonMatch = 1;
//				break;
//			}
//		}
//		//if ( nonMatch )	fprintf( stderr, "\n" );
//
//
//		if ( nonMatch && (cnt < 10) ) {//cnt++ % 1000000 == 1 //&& cnt % 10 == 0
//			cnt++;
//
//			if ( tid != c->tid ) {
//				tid = c->tid;
...
//
//
//			// fetch reference sequence if necessary
//			if ( tid != ref_tid ) {
//				ref = faidx_fetch_seq(fai, target_name[tid], 0, 0x7fffffff, &ref_len); // TODO: CHECK: Is target_name also zero based??
//				//for (i = 0; i < n; ++i) data[i]->ref = ref, data[i]->ref_id = tid0;
//				ref_tid = tid;
//			}
//
//			// ref. sequence
//			fprintf( stderr, "[ref. sequence] = [ " );
//			fprintf( stderr, "\nref. tid(%d), ref. length(%d), ref. pos(%d)\n", ref_tid, ref_len, c->pos );
//			for ( qpos = c->pos; qpos < c->pos + c->l_qseq; qpos++ ) {
//				int base = ref[qpos];
//				int ref0 = bam_nt16_nt4_table[ bam_nt16_table[base] ];
//				fprintf( stderr, "%c(%d)", base, ref0 );
//			}
//			fprintf( stderr, " ]\n" );
//
...
////			if ( strand ) {
////				fprintf( stderr, "(reverse-complimented) " );
////				for ( qpos = c->l_qseq - 1; qpos >= 0; qpos-- ) {
////					int base = bam1_seqi(seq, qpos); // base
////					base = bam_nt16_nt4_table[base? base : ref_base]; // b is the 2-bit base
////					if ( base == 0 ) base = 3;
////					else if ( base == 1 ) base = 2;
////					else if ( base == 2 ) base = 1;
////					else if ( base == 3 ) base = 0;
////					fprintf( stderr, "%c", cbases[base] );
////				}
////			} else {
//				for ( qpos = 0; qpos < c->l_qseq; qpos++ ) {
//					int base = bam1_seqi(seq, qpos); // base
//					base = bam_nt16_nt4_table[base? base : ref_base]; // b is the 2-bit base // TODO: [base? base : ref_base] --> [base]???
//					fprintf( stderr, "%c", cbases[base] );
//				}
////			}
//			fprintf( stderr, " ]\n" );
...
//			for ( qpos = 0; qpos < c->l_qseq; qpos++ ) {
//				int q = qual[qpos]; // quality
//				fprintf( stderr, "%c", q+33 ); // '33' was subtracted when SAM --> BAM
//			}
//			fprintf( stderr, " ]\n" );
//
//			// cigar
...
//
//			// resolve cigar
//			fprintf( stderr, "[cigar-resolved sequence(ref. sequence)] = [ " );
//			s.end = s.k = s.x = s.y = 0;
//			if (c->n_cigar == 1) { // just one operation, save a loop
//				int op = _cop(cigar[0]);
//				int l = _cln(cigar[0]);
...
////					for (k2 = k; k2 < c->n_cigar; ++k2) {
////						int op = _cop(cigar[k2]);
////						int l = _cln(cigar[k2]);
//
//						fprintf( stderr, " %d", l );
//						if ( op == BAM_CMATCH || op == BAM_CEQUAL || op == BAM_CDIFF ) {
...
//
//							for ( qpos=s.x+l2; l2 < l; l2++, qpos++ ) {
//								int base = bam1_seqi(seq, s.y+l2); // base
//								base = bam_nt16_nt4_table[base? base : ref_base]; // b is the 2-bit base
//								fprintf( stderr, "%c(%c) ", cbases[base], ref[qpos] );
//							}
//
//							s.y += l; // common???
//							s.x += l; // common???
...
//							fprintf( stderr, "N" );
//							fprintf( stderr, "(%d,%d): ", s.x, s.y );
//							// TODO: 해당 position 의 ref. base를 N 으로 표시
//							s.x += l; // common???
//							s.y += l; // common???
//						} else if (op == BAM_CINS ) {
//							int l2=0;
...
//							fprintf( stderr, "I" );
//							fprintf( stderr, "(%d,%d): ", s.x, s.y );
//							for ( ; l2 < l; l2++ ) { // TODO: insertion table 에 기록
//								int base = bam1_seqi(seq, s.y+l2); // base
//								base = bam_nt16_nt4_table[base? base : ref_base]; // b is the 2-bit base
//								fprintf( stderr, "%c", cbases[base] );
//							}
...
//
//
//int bam_mplp_auto2(faidx_t *fai, char **target_name, bam_mplp_t iter, int *_tid, int *_pos, int *n_plp, const bam_pileup1_t **plp)
////int bam_mplp_auto(bam_mplp_t iter, int *_tid, int *_pos, int *n_plp, const bam_pileup1_t **plp)
//{
//	int i, ret = 0;
//	for (i = 0; i < iter->n; ++i) {
//		int tid, pos;
//		bam_plp_auto2(fai, target_name, iter->iter[i], &tid, &pos, &iter->n_plp[i]);
//		fprintf( stderr, "### END of one file ###\n" );
//		//iter->pos[i] = (uint64_t)tid<<32 | pos;
//		ret = 0; // TODO: need to change value of 'ret'
//	}
//	return ret;
//}
